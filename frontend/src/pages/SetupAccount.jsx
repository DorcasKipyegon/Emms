...
export default function SetupAccount() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'Account activated successfully! Redirecting to login...' });
      ...
      setMessage({ type: 'error', text: err.response?.data?.error || 'Invalid or expired setup link.' });
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-teal-50/50 to-slate-100 flex flex-col justify-center items-center py-12 px-6">
        <div className="bg-white p-8 rounded-[2rem] shadow-xl text-center max-w-sm w-full">
          ...
          <p className="text-sm text-gray-500 mb-6">This account setup link is missing required parameters or has expired.</p>
          <Link to="/login" className="text-teal-600 font-bold hover:text-teal-500">Go to Login</Link>
        </div>
      </div>
    );
  }

...
export default function InviteWorkerModal({ onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post('users/invite_worker/', formData);
      onSuccess();
    } catch (err) {
      setError(err.response?.data?.error || 'Failed to invite Worker. Please check the details and try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center shrink-0">
          <div className="flex items-center text-slate-700">
            ...
            <h3 className="font-bold text-lg">Invite Worker</h3>
          </div>
        ...
        <div className="p-6 overflow-y-auto">
          ...
          <p className="text-sm text-gray-500 mb-6">
            An invitation email will be sent to this Worker with a secure link to set up their account password.
          </p>

          <form id="invite-Worker-form" onSubmit={handleSubmit} className="space-y-4">
